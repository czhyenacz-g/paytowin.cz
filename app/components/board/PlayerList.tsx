                          ...
                          return (
                            ...
                            >
                              <div className={`flex items-start gap-2 text-sm font-semibold leading-snug ${h.isPreferred ? "text-amber-700" : "text-slate-700"}`}>
                                {h.image
                                  ? ( // eslint-disable-next-line @next/next/no-img-element
                                    <img src={h.image} alt={h.name} title={h.flavorText} className="mt-0.5 h-6 w-6 shrink-0 rounded object-cover bg-slate-100 cursor-help" onError={(e) => { e.currentTarget.style.display = "none"; }} />)
                                  : <span className="mt-0.5 shrink-0 text-base" title={h.flavorText}>{h.emoji}</span>
                                }
